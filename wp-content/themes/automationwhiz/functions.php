...
function cf7_validate_api($cf7, &$abort, $submission) {
    ...
    {        
		...
		$url = "http://staging.usercentral.testing-whiz.com/regservice/".$versionid;  //Staging
		//$url = "https://usercentral.testing-whiz.com/regservice/".$versionid;  // Live
		$curl = curl_init();
		...
		if($response == 1){
			...
			$errMsg = "Unable to process the request at this movement. Please contact us over an email.";
			$abort = true;
			$submission->set_response($cf7->message('validation_failed'));
			$submission->set_response($cf7->filter_message($errMsg)); //custom msg;
		}		
