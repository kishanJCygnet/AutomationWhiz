...
class Schema extends CommonSchema\Schema {
	...
	public function get() {
		...
		// Now, let's also grab all the schema supported blocks if this a post.
		$schema['@graph'] = array_merge( $schema['@graph'], $this->getBlockGraphs() );
	...
	private function getBlockGraphs() {
		...
		if ( empty( $metaData->schema_type_options ) ) {
			return [];
		}

		$schemaTypeOptions = json_decode( $metaData->schema_type_options );
		if ( empty( $schemaTypeOptions->blockGraphs ) ) {
			return [];
		}
		...
		foreach ( $schemaTypeOptions->blockGraphs as $blockGraph ) {
			// If the type isn't set for whatever reason, then bail.
			if ( empty( $blockGraph->type ) ) {
				continue;
			}

			$type = strtolower( $blockGraph->type );
			switch ( $type ) {
				case 'aioseo/faq':
					...
					$faqPages[] = $faqPageInstance->get( $blockGraph );
					break;
				default:
					break;
			}
